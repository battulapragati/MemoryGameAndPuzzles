...
RIGHT = 'right'
time_left=0
PLAY_TIME =300 #edit this
PLAY_TIME +=13#dont edit this
def draw_timer(screen, x, y, time_left):
    font = pygame.font.Font(None, 36) #Choose the font for the text
    text = font.render(time_left, 1, WHITE) #Create the text
...
def main():
    ...
    while True: # main game loop
        ...
        minutes = time_left // 60          # Use modulus (remainder) to get seconds
        seconds = time_left % 60
        if time_left<0:
             resetAnimation(mainBoard, allMoves)
             #surf=BASICFONT.render("GAME OVER",True,DARKTURQUOISE)
             #titleRect=surf.get_rect()
        ...
        draw_timer(DISPLAYSURF, 550, 80,output_string ) #Once we have calculated how much time is left, draw the timer
